...
class Worker:
    ...
    def load_stock(self):
        ...
        self.stock = bool('out' not in parser.find('meta', {'property': 'og:availability'}).get('content').strip())
    ...
    def write_csv(self):
        ...
        with open(f'{file_dir}/data/{self.num}.csv', 'a', encoding='utf-8', newline='') as file:
            ...
            writer.writerow([time.strftime('%m/%d'), time.strftime('%H:%M'), 'O' if self.stock else 'X'])
...
if __name__ == '__main__':
    ...
    with open(f'{file_dir}/products.html', 'w', encoding='utf-8') as file:
        ...
        for worker in workers:
            if worker.stock is None:
                stock = '<div><b>정보 없음</b></div>'
            elif worker.stock:
                stock = '<div style="color: green"><b>재고 있음</b></div>'
            else:
                stock = '<div style="color: red"><b>품절</b></div>'

            grid += f'<div class="grid_item"><a href="table.html?n={worker.num}&l={urllib.parse.quote(base64.b64encode(worker.url.split("/")[-2].encode("utf-8")).decode("utf-8"))}"><img src="{worker.image}"><div>{worker.name}</div><div>{stock}</div></a></div>\n'
